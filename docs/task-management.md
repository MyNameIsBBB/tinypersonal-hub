...
## Task Focus and Daily Briefing

`getTaskFocus` is the single deterministic priority engine used by the AI tool, prompt context, and daily briefing. It returns `overdue`, `dueToday`, `upcoming`, `inProgress`, and `recommended` groups. Ranking combines deadline urgency, overdue age, the explicit task priority, and active/blocked status; the language model formats this result but must not recalculate or reorder it.

Each focus item includes the score reasons, days to/from deadline, checklist totals, and up to five remaining checklist labels. The morning briefing is generated for the configured authenticated owner and includes today's schedule, recommended work, overdue warnings, news, and market data. Push delivery is owner-scoped.
