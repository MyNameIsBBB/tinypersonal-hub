...
function morningMessageId(cycleDate: string): string { return `morning-${cycleDate}`; }

async function deterministicMorningBriefing(cycleDate: string): Promise<string> {
  const start = new Date(`${cycleDate}T00:00:00+07:00`);
  const end = new Date(start.getTime() + 86_400_000 - 1);
  const schedule = await getScheduleByRange(start, end);
  if (!schedule.length) return `Good morning ครับ Sir — วันนี้ (${cycleDate}) ตารางยังว่าง ไม่มีนัดหมายหรืองานที่บันทึกไว้ครับ`;
  const lines = schedule.slice(0, 12).map((item) => {
    const time = item.startTime?.toLocaleTimeString("th-TH", { timeZone: "Asia/Bangkok", hour: "2-digit", minute: "2-digit" }) ?? "ไม่ระบุเวลา";
    return `• ${time} ${item.title}`;
  });
  return `Good morning ครับ Sir — ตารางวันนี้ (${cycleDate}) มี ${schedule.length} รายการ:\n${lines.join("\n")}`;
}

async function resetGeneralSession(sessionId: string, cycleDate: string, message: string, replaceExisting = false): Promise<void> {
  const id = morningMessageId(cycleDate);
  const existing = await prisma.chatMessage.findUnique({ where: { sessionId_messageId: { sessionId, messageId: id } }, select: { id: true } });
  if (existing && !replaceExisting) return;
  const payload: StoredChatMessage = { id, role: "assistant", parts: [{ type: "text", text: message }] };
  await prisma.$transaction([
    prisma.chatMessage.deleteMany({ where: { sessionId, messageId: { not: id } } }),
    prisma.chatMessage.upsert({
      where: { sessionId_messageId: { sessionId, messageId: id } },
      create: { sessionId, messageId: id, role: "assistant", payloadJson: JSON.stringify(payload) },
      update: { role: "assistant", payloadJson: JSON.stringify(payload) },
    }),
    prisma.chatSession.update({ where: { id: sessionId }, data: { title: GENERAL_CHAT_TITLE } }),
  ]);
}

export async function ensureDailyGeneralChat(ownerKey: string, now = new Date()) {
  const cycleDate = bangkokCycleDate(now);
  let session = await prisma.chatSession.findFirst({ where: { ownerKey, title: GENERAL_CHAT_TITLE }, orderBy: { createdAt: "asc" } });
  session ??= await prisma.chatSession.create({ data: { ownerKey, title: GENERAL_CHAT_TITLE } });
  await resetGeneralSession(session.id, cycleDate, await deterministicMorningBriefing(cycleDate));
  return session;
}

export async function resetAllGeneralChats(message: string, cycleDate: string): Promise<number> {
  const sessions = await prisma.chatSession.findMany({ where: { title: GENERAL_CHAT_TITLE }, select: { id: true } });
  await Promise.all(sessions.map(({ id }) => resetGeneralSession(id, cycleDate, message, true)));
...
export async function replaceChatMessages(ownerKey: string, sessionId: string, messages: StoredChatMessage[]) {
  ...
  const title = session.title === GENERAL_CHAT_TITLE ? GENERAL_CHAT_TITLE : retained.find((message) => message.role === "user")?.parts
...
export async function deleteChatSession(ownerKey: string, sessionId: string) {
  return prisma.chatSession.deleteMany({ where: { id: sessionId, ownerKey, title: { not: GENERAL_CHAT_TITLE } } });
}
