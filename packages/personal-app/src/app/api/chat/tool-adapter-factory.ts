...
function scheduleStatusTool(ownerKey: string, sessionId: string) {
  return tool({
    ...
    execute: async (input) => pendingResult(ownerKey, sessionId, "schedule.updateStatus", `อัปเดตสถานะงาน ${input.id} เป็น ${input.status}`, input),
  });
}
...
const webSearchExecutionTool = tool({
  ...
  execute: async ({ query, count }) => {
    try {
      ...
      return { ok: false as const, error: { code: "WEB_SEARCH_FAILED", message: error instanceof Error ? error.message : "Web search failed" } };
    }
  },
});
...
const webScrapeExecutionTool = tool({
  ...
  execute: async ({ url, maxCharacters }) => {
    try {
      ...
      return { ok: false as const, error: { code: "WEB_SCRAPE_FAILED", message: error instanceof Error ? error.message : "Web scrape failed" } };
    }
  },
});
...
function codingDelegateTool(ownerKey: string, sessionId: string, originalUserMessage: string) {
  return tool({
    description: toolContracts.delegateCodingTask.description,
    inputSchema: toolContracts.delegateCodingTask.input,
    execute: async (input) => pendingResult(
      ownerKey,
      sessionId,
      "coding.delegateTask",
      `ส่งงานให้ Codex: ${originalUserMessage.slice(0, 180)}`,
      { ...input, instruction: originalUserMessage },
    ),
  });
}
...
async function pendingResult(
  ...
  return confirmationResult(await createPendingAction({
    ownerKey,
    sessionId,
    toolName,
    summary,
...
export function createChatTools(
  ...
  const adapters = {
    ...
    delegateCodingTask: codingDelegateTool(ownerKey, sessionId, originalUserMessage),
